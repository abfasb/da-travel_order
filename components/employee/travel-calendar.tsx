  ...
  return (
    <Card className="w-full shadow-lg border-0 overflow-hidden">
      <CardHeader className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white p-6">
        ...
        <p className="text-emerald-100 text-sm mt-1">Your scheduled travels and official trips</p>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">
              ...
              <div className="flex items-center gap-1"><div className="w-3 h-3 bg-amber-100 border border-amber-300 rounded-full"></div><span>Today</span></div>
            </div>
          </div>
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <h4 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                <Plane className="h-4 w-4 text-emerald-600" /> Upcoming travels
              </h4>
              <ScrollArea className="h-40 pr-3">
                <div className="space-y-2">
                  {upcomingTravels.map((travel) => (
                    <div key={travel.id} className="flex items-start gap-2 p-2 rounded-lg hover:bg-emerald-50 transition-colors cursor-pointer" onClick={() => setSelectedDay(travel.date)}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 text-xs text-gray-500"><MapPin className="h-3 w-3" /><span className="truncate">{travel.destination}</span></div>
                        <p className="text-xs text-gray-400">{format(travel.date, 'MMM d, yyyy')}</p>
                      </div>
                  ...
                  {upcomingTravels.length === 0 && <p className="text-sm text-gray-400 text-center py-4">No upcoming travels</p>}
                </div>
              </ScrollArea>
            </div>
            <Separator />
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              <h4 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                <Clock className="h-4 w-4 text-emerald-600" /> {selectedDay ? format(selectedDay, 'MMMM d, yyyy') : 'Select a day'}
              </h4>
                  ...
                  <div className="space-y-3">
                    {eventsForSelectedDay.map((event) => (
                      <div key={event.id} className="border-l-2 border-emerald-400 pl-3">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 mt-1"><MapPin className="h-3 w-3" /><span>{event.location}, {event.destination}</span></div>
