...
import { notifyHR } from '@/lib/notification-helper'
...
export async function submitApproval({
  ...
  try {
    ...
    if (action === 'APPROVE' && allApproved) {
      await notifyHR(
        'Travel order ready for processing',
        `Travel order ${travelOrderNumber} has been fully approved and is ready for number assignment.`,
        `/hr/orders/${approval.travelOrderId}`,
        approval.travelOrderId,
        'HR_READY'
      )
    }
